...
#define EXIT_CODE_MASK		0x0000ffff
typedef enum {
	EXIT_NORMAL = 0,
	EXIT_WITH_PROMPT = EXIT_CODE_MASK & (1 << 0),
	EXIT_DONT_BLOCK = EXIT_CODE_MASK & (1 << 1),
	EXIT_COUNT_OUT = EXIT_CODE_MASK & (1 << 2),
} ExitCode;

#define ERROR_CODE_MASK		0xffff0000
#define ERR_FLAG			0x00010000
#define HAS_ERROR(code)		((ERROR_CODE_MASK&(code))!=0)
typedef enum {
	ERR_EXIT_WITH_PROMPT = ERROR_CODE_MASK & (ERR_FLAG << 0),
	ERR_OPEN_DEVICE = ERROR_CODE_MASK & (ERR_FLAG << 1),
	ERR_INOTIFY_ADD_WATCH = ERROR_CODE_MASK & (ERR_FLAG << 2),
	ERR_SCAN_DIR = ERROR_CODE_MASK & (ERR_FLAG << 3),
	ERR_GET_SWITCH_STATE = ERROR_CODE_MASK & (ERR_FLAG << 4),
	ERR_READ_EVENT = ERROR_CODE_MASK & (ERR_FLAG << 5),
} ErrorCode;

/**
 * map the parse methods, it'll not be invoked if NULL.
 */
static int (*devFilter)(const char*) = nativeDevFilter;
static void (*addToEventHub)(const char*) = nativeAddToEventHub;
static const char* (*getDevEvent)(void) = nativeGetDevEvent;
static void (*setAbsEvent)(int, int) = nativeSetAbs;
static int (*getCalibratedWidth)(void) = nativeGetCalibratedWidth;
static int (*getCalibratedHeight)(void) = nativeGetCalibratedHeight;
static int (*parseEvent)(struct input_event) = nativeParseEvent;

/**
 *  src: /android/4.3/system/core/toolbox/getevent.c
 *  modified:
 *    - set log;
 *    - set func pointer to callback native parse methods, see above;
 *    - wrapped with native-prefix method;
 */
#define SRC_VERSION_ANDROID_4_3
				...
				if (bits[j] & 1 << k) {
					...
					if (i == EV_ABS) {
						...
						if (ioctl(fd, EVIOCGABS(j * 8 + k), &abs) == 0) {
							...
							if (setAbsEvent != NULL) {
								setAbsEvent(j * 8 + k, abs.maximum); // TODO: j*8+k must be event.code
							}
						}
					}
					count++;
...
static int open_device(const char *device, int print_flags) {
	...
	if (devFilter != NULL) {
		if (devFilter(name)) {
			if (addToEventHub != NULL) {
				addToEventHub(device);
			}
		}
	}
...
static int getevent_main(int argc, char *argv[]) {
	int c;
	int i;
	int res;
	int pollres;
			...
			if (ufds[i].revents) {
				if (ufds[i].revents & POLLIN) {
					...
					if (res < (int) sizeof(event)) {
						...
						return ERR_READ_EVENT;
					} else {
						/*
						 * start to parse event...
						 */
						LOGV("[%8ld.%06ld] %04x %04x %08x",
...
int native_init() {
	...
	nativeDebug("[%p] halfway %s", ret,
			!HAS_ERROR(ret) ? "ok." : "oops!");

	if (HAS_ERROR(ret)) {
		LOGE("failed to init, error code: %d", ret);
	} else {
		if (getDevEvent != NULL
				&& getCalibratedWidth != NULL
				&& getCalibratedHeight != NULL) {
			nativeDebug("event: %s, calibrated axis %dx%d", getDevEvent(),
					getCalibratedHeight(), getCalibratedWidth());
		}
	}

	nativeDebug("native init ok.");
	return 0;
}

int native_getevent() {
	nativeDebug("native get event...");

	char* argv[2] = { "getevent", "/dev/input/event0" };
	const int argc = 2;
	...
	int ret = getevent_main(argc, argv);
	if(HAS_ERROR(ret)) {
		LOGE("failed to get event, error code: %d", ret);
	}

	nativeDebug("native get event stop.");
	return 0;
}
